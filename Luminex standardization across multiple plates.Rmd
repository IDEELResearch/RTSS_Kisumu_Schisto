      ...
      standards_plate_stats <- data.frame(
        ...
        R_squared = round(correlation$r_squared, 2)
          ...
          #Reextrapolate Subclass from Plate column
            standards_plate_stats$Subclass <- sub("^[^_]+_([^_]+)_.*$", "\\1", standards_plate_stats$Plate)
          # trim dataframe
            standards_plate_stats <- standards_plate_stats %>% select(Plate, Slope, Y_intercept, Fit_line, R_squared, 
                                                                      Subclass)
        # show columns
...
The average line of best for each `Subclass` will be determined and listed below (this assumes both the plate and subclass type are the causes for variation)

### 3. Determine scaling factor

Select a `Dilution_Factor` within the linear log10(MFI) region of the `standard_antigen` of the standard curve. The `plate_scaling_factor` is determined as the ratio between the values of the plate line of best fit and the average line of best fit (Subclass specific) at that dilution. 
...
# No input required
     # Join the average slope and intercept from average_fit_lines to standards_plate_stats based on matching Subclass values
      standards_plate_stats <- standards_plate_stats %>%
      ...
      standards_plate_stats$bestfit_avg_ln <- standards_plate_stats$average_slope*scaling_dilution_factor + standards_plate_stats$average_intercept
      
      standards_plate_stats$scaling_factor <- standards_plate_stats$bestfit_avg_ln/standards_plate_stats$bestfit_plate
...
This assumes *linearity* of the correlation. The `bestfit_plate` is the `Fit_line` calculation for `scaling_dilution_factor`. The `bestfit_avg_ln` is the same calculation for the `average_fit_line` for each `Subclass`. The `scaling_factor` will be used on the MFI data from each plate in the compiled dataframe
