  ...
  # Creation of standards dataframes ----
      #standards_df: all standards data from all paltes
          ...
          #Create Plate column
             ...
             # Create Antigen and Serology columns based on Ab_Ag column
                standard_mfi_df <- standard_mfi_df %>%
                  mutate(
                    Serology = sub("_.+", "", Ab_Ag),  # Extract what's before the "_"
...
# Generate plots ----
    ...
    ggplot(standard_mfi_df, aes(x = Dilution_Factor, y = log10(MFI), color = Plate)) +
  geom_point() +
...
```{r Antigen standard curves, echo=FALSE, warning=FALSE}
 ...
 plots <- standard_mfi_df %>%
  ...
  map(~ ggplot(.x, aes(x = Dilution_Factor, y = log10(MFI), color = Plate)) +
        geom_point() +
        labs(x = "Log 10 Dilution Factor", y = "MFI", color = "Plate") +
...
# Standardization
For our study, I think we should use `AMA1` only. These are the plots based on `Serology` type. It seems serology influences the behavior of the curves but all seem to respond logarithmically. 
...
# Create scatterplot with filtered data and facet by Serology
ama1_plot <- ggplot(ama1_data, aes(x = Dilution_Factor, y = log10(MFI))) +
  geom_point(aes(color = Serology)) +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE) +  # Add logarithmic line of best fit
  labs(x = "Dilution Factor", y = "log10(MFI)", color = "Serology") +
  ggtitle("MFI Standard Curve for AMA1 based on Serology") +
  theme_minimal() +
  facet_wrap(~ Serology, scales = "free_y")
...
## Standardization based on Serology type for AMA1

```{r}
# Split data by Serology
ama1_data_split <- split(ama1_data, ama1_data$Serology)

# Create a list to store plots
ama1_plots <- list()

# Create scatterplot for each Serology level with a logarithmic regression line
for (serology_level in names(ama1_data_split)) {
  ama1_plots[[serology_level]] <- ggplot(ama1_data_split[[serology_level]], aes(x = Dilution_Factor, y = log10(MFI), color = Plate)) +
    geom_point(na.rm = TRUE) +
    geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE, na.rm = TRUE) + # Add logarithmic regression line
    labs(x = "Dilution Factor", y = "log10(MFI)", color = "Plate") +
    ggtitle(paste("MFI Standard Curve for AMA1 - Serology:", serology_level)) +
    theme_minimal()
}
