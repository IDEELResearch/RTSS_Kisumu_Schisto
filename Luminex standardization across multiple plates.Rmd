  ...
  # Creation of standards dataframes ----
      #standards_df: all standards data from all paltes
        ...
        standard_mfi_df <- tidyr::gather(standards_df, key = "Ab_Ag", value = "MFI", all_of(standard_mfi_cols))
              ...
              # Filter out rows where MFI is NA
          ...
          # Create Antigen groups
            ...
            (Plate = apply(standard_mfi_df[plate_cols], 1, get_plate_value))
        ...
        standard_mfi_df <- standard_mfi_df %>%
          select(Plate, Subclass, Antigen, MFI, log10_MFI)
...
# No input required
      ...
      standards_plate_stats <- data.frame(
        Plate = correlation$Plate, Subclass= standard_filtered_data$Subclass,Slope=correlation$slope, Y_intercept = correlation$intercept,
